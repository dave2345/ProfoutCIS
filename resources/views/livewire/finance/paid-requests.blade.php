<div>
    <div class="mb-4 flex justify-between items-center">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Paid Requests</h2>
        <div class="flex space-x-2">
            <button
                wire:click="exportPaid"
    ...
    <div class="mb-4 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="col-span-2">
            <input
                ...
                placeholder="Search by request number, description, or user..."
        ...
        <div>
            <select wire:model.live="dateFilter" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Total Paid Amount</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">${{ number_format($totalPaidAmount, 2) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">${{ number_format($monthlyPaidAmount, 2) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $avgProcessingTime }} days</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-900">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300" wire:click="sortBy('paid_at')">
                        Paid Date
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($requests as $request)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    @if($request->user->profile_photo_path)
                                        <img class="h-10 w-10 rounded-full" src="{{ Storage::url($request->user->profile_photo_path) }}" alt="">
                                    @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            <div>{{ $request->paid_at ? $request->paid_at->format('M d, Y') : 'N/A' }}</div>
                            <div class="text-xs text-gray-400">{{ $request->paid_at ? $request->paid_at->format('h:i A') : '' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-3">
                                <button
                                    wire:click="markAsApproved({{ $request->id }})"
                                    wire:confirm="Are you sure you want to mark this request as approved?"
                                ...
                                <button
                                    wire:click="downloadReceipt({{ $request->id }})"
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-500 dark:text-gray-400 text-lg">No paid requests found</p>
                                <p class="text-gray-400 dark:text-gray-500 text-sm mt-2">Paid requests will appear here once processed</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
